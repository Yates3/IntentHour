...
| 当前状态 | staging 已部署，可注册、可付款沙盒演示；生产正式域名与生产商户审核待确认 |
| 开始时间 | 2026-07-18，可由初始 Git 历史与迁移/文档时间推断 |
| 最近更新时间 | 2026-07-22 |
| GitHub 仓库 | https://github.com/Yates3/IntentHour.git |
| 在线体验地址 | https://intenthour-staging.ylin99207.workers.dev |
| 正式域名 | 待确认 |
...
* 已验证结果：README 与 deployment 文档记录 Google OAuth staging 可用。
* 已知限制：magic link 在 staging provider status 中仍依赖 Resend 域名/API key。
...
* 最新已确认 staging Worker version：`476b408d-cea3-468a-8e7e-00fcd678a521`
* 冒烟检查：2026-07-22，`/app` 返回 HTTP 200
...
| --- | --- | --- | --- | --- | --- |
| 正式域名、生产 provider、Paddle 商户审核待确认 | 高 | 生产发布 | 是 | 完成域名、OAuth、Turnstile、Resend、Paddle production 配置 | P0 |
...
staging 已部署到 Cloudflare Worker：`https://intenthour-staging.ylin99207.workers.dev`。最新已确认 Worker version：`476b408d-cea3-468a-8e7e-00fcd678a521`。生产正式域名待确认。
...
正式域名、正式联系邮箱、法律主体/退款说明最终文本、Resend 发送域名、Paddle 生产审核状态、是否需要公开演示视频、是否要启用自动 CI/CD。
...
staging Worker: intenthour-staging，URL: https://intenthour-staging.ylin99207.workers.dev，D1: intenthour-staging。最新已确认部署 version 是 476b408d-cea3-468a-8e7e-00fcd678a521。生产正式域名和 provider production 配置待确认。
...
源码中仍有少量 UI 文案 mojibake 符号；staging D1 目前确认 completed sessions 为 2，需原 Pro 浏览器刷新回填第三条；Resend magic link 生产发送域名待确认；正式域名和 Paddle 生产审核未完成；时间到自动结束未实现。
