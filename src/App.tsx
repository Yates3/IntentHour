...
export default function App() {
  ...
  const syncAfterCompletion = useCallback(async () => {
    if (entitlement.pro) await syncCompletedSessions();
  }, [entitlement.pro]);
  ...
  if (pathname.startsWith("/app")) return <AppShell view={view} navigate={navigate} pro={entitlement.pro}>{view === "focus" ? <FocusPage onSessionCompleted={syncAfterCompletion} /> : view === "sessions" ? <SessionsPage /> : view === "patterns" ? <PatternsPage pro={entitlement.pro} navigate={navigate} /> : <SettingsPage pro={entitlement.pro} refreshEntitlement={refresh} navigate={navigate} />}</AppShell>;
